...
select = dtage[sig=="FWER < 0.05" & nearestSymbol%in%c("SHH","TET3","SMAD9","VANGL2","DCHS1","CLSTN1","MAPK8IP1","MAPK8IP3"),,]
coord = as.list(unique(select$regionID))
names(coord) = as.character(as.data.frame(select)[match(unlist(coord),select$regionID),"nearestSymbol"])
ov = lapply(coord, function(x) findOverlaps(gr,GRanges(x)))
means = c(lapply(ov[which(names(ov)!="SMAD9")], function(x) colMeans(meth[queryHits(x),])), 
          lapply(ov[which(names(ov)=="SMAD9")], function(x)  meth[queryHits(x),]))
names(means[[1]])
...
fwer = lapply(fwer, function(x) reduce(makeGRangesFromDataFrame(x)))
