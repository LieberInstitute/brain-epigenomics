# Plot LDSC results for 'adjusting for stringent' analyses
...
categories_df <- categories_df %>%
  ...
  mutate(Category = factor(Category, Category, ordered = TRUE),
         Extended = factor(Extended, Extended,
                           ordered = TRUE)) %>%
  arrange(Extended)

categories_df = data.frame(categories_df)
categories_df$`Proportion of SNPs` = data.frame(snp_prop_table)[match(categories_df$Category, data.frame(snp_prop_table)$Category),"Proportion.of.SNPs"]
categories_df$`Proportion of CpGs` = data.frame(cpg_prop_table)[match(categories_df$Category, data.frame(cpg_prop_table)$Category),"Proportion.of.CpGs"]
...
fls <- fls[-c(grep("Agreeableness", fls), grep("Cardioembolic_stroke", fls), grep("Large-vessel_disease", fls), grep("Small-vessel_disease", fls))]

# Join munged LDSC output with categories_df and traits_df
x <- cbind(data.frame(x), categories_df[match(x$Category, categories_df$Category),colnames(categories_df)!="Category"])
x <- cbind(x, traits_df[match(x$Trait, traits_df$Trait),colnames(traits_df)!="Trait"])
rownames(x) = NULL

stopifnot(length(fls) == nrow(x))

write.csv(x, file = "/dcl01/lieber/ajaffe/lab/brain-epigenomics/rdas/ldsc/tables/stringent_ldsc_output_allIncluded.csv", quote=F)
x = read.csv("/dcl01/lieber/ajaffe/lab/brain-epigenomics/rdas/ldsc/tables/stringent_ldsc_output_allIncluded.csv")
...
write.csv(x, file = "/dcl01/lieber/ajaffe/lab/brain-epigenomics/rdas/ldsc/tables/stringent_ldsc_output.csv", quote=F)
...
x = read.csv("/dcl01/lieber/ajaffe/lab/brain-epigenomics/rdas/ldsc/tables/stringent_ldsc_output.csv")
...
x_stratified %>%
  ...
  rename(Feature = Extended,
         ...
         Stratum = `strata`) %>%
  write_csv("/dcl01/lieber/ajaffe/lab/brain-epigenomics/rdas/ldsc/tables/LDSC_results.stringent_adjustments.csv")
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Exploratory_boxplots_byFeature_stringent.pdf",
    height = 3.5, width = 10)
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Exploratory_boxplots_byTrait_stringent.pdf",
    height = 4.5, width = 16)
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Coefficient_Z-score.stringent_adjustments.pdf",
    height = 6.5, width = 14)
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Coefficient_Z-score.stringent_adjustments.stratified.pdf",
    height = 4, width = 5)
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Enrichment.stringent_adjustments.pdf", height = 8.5, width = 10)
...
pdf("/dcl01/lieber/ajaffe/lab/brain-epigenomics/LDSC/figures/Enrichment.stringent_adjustments.sig_stratified.pdf", height = 4, width = 5)
