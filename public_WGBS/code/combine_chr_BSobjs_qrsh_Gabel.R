...
for (i in 1:length(chrs)) {
  ...
  save(BSobj, file = gsub('_cleaned_CX_Gabel.Rdata', '_cleaned_Gabel_CpG_coerced.Rdata', f))
  message(paste(Sys.time(), 'saving coerced CHs: chr', chrs[i]))
  BSobj = x[granges(x)$c_context!="CG",]
  save(BSobj, file = gsub('_cleaned_CX_Gabel.Rdata', '_cleaned_Gabel_CpH_coerced.Rdata', f))
