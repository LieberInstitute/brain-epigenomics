...
#$ -N step2-trim-Stroud_Greenberg_ChIPseq.Aug212018
#$ -pe local 4
#$ -o ./logs/trim-Stroud_Greenberg_ChIPseq.$TASK_ID.txt
#$ -e ./logs/trim-Stroud_Greenberg_ChIPseq.$TASK_ID.txt
#$ -t 1-0
#$ -tc 5
#$ -hold_jid pipeline_setup,step1-fastqc-Stroud_Greenberg_ChIPseq.Aug212018
#$ -m a
echo "**** Job starts ****"
date
...
if [ FALSE == "TRUE" ] ; then 
	REPORT1=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Untrimmed/${ID}/${FILEBASE1}_fastqc/summary.txt
	REPORT2=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Untrimmed/${ID}/${FILEBASE2}_fastqc/summary.txt
	...
	if [[ $RESULT1 == "FAIL" || $RESULT2 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq
		FP=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq/${ID}_trimmed_forward_paired.fastq
		FU=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq/${ID}_trimmed_forward_unpaired.fastq
		RP=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq/${ID}_trimmed_reverse_paired.fastq
		RU=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq/${ID}_trimmed_reverse_unpaired.fastq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc 		$FP $FU $RP $RU 		--outdir=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Trimmed/${ID} --extract
	else
	...
	if [[ $RESULT1 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq
		OUT=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/trimmed_fq/${ID}_trimmed.fastq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc $OUT 		--outdir=/dcl01/lieber/ajaffe/lab/brain-epigenomics/public_WGBS/code/FastQC/Trimmed/${ID} --extract
	else
		echo "No trimming required!"
	fi
fi
