...
opt <- list('organism' = 'hg19',
	'maindir' = '/dcl01/lieber/ajaffe/lab/brain-epigenomics/brainseq_pipeline/riboZero_stranded',
	'experiment' = 'ribozero',
	'prefix' = 'dlpfc',
	'paired' = TRUE,
	'stranded' = 'reverse',
...
## added manually -- include sample_information.csv in metrics ##
sample_info = read.csv("../sample_information.csv", stringsAsFactors=FALSE, row.names=1)
sample_info = sample_info[sample_info$Experiment == "RiboZero" ,]
sample_info = sample_info[metrics$SAMPLE_ID,]
colnames(sample_info)[9] = "Samples"

metrics = cbind(sample_info, metrics)
##
...
colnames(txNumReads) = colnames(txTpm) = rownames(metrics)
############################################################ 
...
colnames(geneCounts) = colnames(exonCounts) = rownames(metrics)
colnames(geneRpkm) = colnames(exonRpkm) = rownames(metrics)
...
### save counts
colnames(jCounts) = colnames(jRpkm) = rownames(metrics)
